...
export function LandingChatbot() {
  ...
  return (
    <>
      {/* WhatsApp Floating Button - IZQUIERDA en móvil */}
      <button
        onClick={openWhatsApp}
        className="fixed bottom-6 left-6 md:left-auto md:right-6 z-[60] p-4 bg-green-500 hover:bg-green-600 text-white rounded-full shadow-2xl hover:scale-110 transition-all duration-200 group"
      ...
      {/* Chatbot Widget Button - CENTRO en móvil */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className={`fixed bottom-6 left-1/2 -translate-x-1/2 md:translate-x-0 md:left-auto md:right-24 z-[60] p-4 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white rounded-full shadow-2xl hover:scale-110 transition-all duration-200 ${
          isOpen ? 'rotate-90' : ''
        }`}
