  ...
  try {
    ...
    const metrics = {
      ...
      eventos: {
        ...
        tendencia: previousEvents > 0 ? ((events - previousEvents) / previousEvents * 100).toFixed(1) : 0
      },
      posts: {
        total: posts._count,
        totalLikes: posts._sum?.likes || 0,
        totalComentarios: comments,
        tendencia: previousPosts > 0 ? ((posts._count - previousPosts) / previousPosts * 100).toFixed(1) : 0
      },
      anuncios: {
        total: announcements
      },
