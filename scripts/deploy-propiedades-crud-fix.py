#!/usr/bin/env python3
"""
Script de Deployment para fix de CRUD de propiedades
Usa Paramiko para conexión SSH
"""

import sys
import time
import os

# Add user packages path
sys.path.insert(0, '/home/ubuntu/.local/lib/python3.12/site-packages')

import paramiko

# Configuración
SERVER_IP = "157.180.119.236"
SERVER_USER = "root"
SERVER_PASSWORD = os.getenv("SSH_PASSWORD", "hBXxC6pZCQPBLPiHGUHkASiln+Su/BAVQAN6qQ+xjVo=")
APP_PATH = "/opt/inmova-app"

class Colors:
    GREEN = '\033[92m'
    RED = '\033[91m'
    YELLOW = '\033[93m'
    CYAN = '\033[96m'
    END = '\033[0m'

def log(message, color=None):
    timestamp = time.strftime('%H:%M:%S')
    if color:
        print(f"{color}[{timestamp}] {message}{Colors.END}")
    else:
        print(f"[{timestamp}] {message}")

def exec_cmd(client, command, timeout=300):
    """Ejecutar comando SSH y retornar resultado"""
    stdin, stdout, stderr = client.exec_command(command, timeout=timeout)
    exit_code = stdout.channel.recv_exit_status()
    output = stdout.read().decode('utf-8', errors='ignore')
    error = stderr.read().decode('utf-8', errors='ignore')
    return exit_code, output, error

def main():
    print("=" * 70)
    log("🚀 DEPLOYMENT: Fix CRUD Propiedades (Eliminar)", Colors.CYAN)
    print("=" * 70)
    print(f"Servidor: {SERVER_IP}")
    print(f"Path: {APP_PATH}")
    print("=" * 70)
    
    # Conectar SSH
    log("🔐 Conectando al servidor...", Colors.YELLOW)
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    
    try:
        client.connect(
            SERVER_IP, 
            username=SERVER_USER, 
            password=SERVER_PASSWORD, 
            timeout=30
        )
        log("✅ Conectado al servidor", Colors.GREEN)
    except Exception as e:
        log(f"❌ Error de conexión: {e}", Colors.RED)
        return 1
    
    try:
        # 1. Verificar directorio
        log("📁 Verificando directorio...", Colors.YELLOW)
        exit_code, output, error = exec_cmd(client, f"cd {APP_PATH} && pwd")
        if exit_code != 0:
            log(f"❌ Directorio no encontrado: {error}", Colors.RED)
            return 1
        log(f"✅ Directorio: {output.strip()}", Colors.GREEN)
        
        # 2. Pull de cambios
        log("📥 Actualizando código...", Colors.YELLOW)
        
        exit_code, output, error = exec_cmd(client, f"cd {APP_PATH} && git fetch origin")
        
        exit_code, output, error = exec_cmd(
            client, 
            f"cd {APP_PATH} && git checkout cursor/proyecto-inmova-despliegue-032a 2>/dev/null || git checkout -b cursor/proyecto-inmova-despliegue-032a origin/cursor/proyecto-inmova-despliegue-032a"
        )
        
        exit_code, output, error = exec_cmd(
            client, 
            f"cd {APP_PATH} && git pull origin cursor/proyecto-inmova-despliegue-032a"
        )
        log("✅ Código actualizado", Colors.GREEN)
        
        # Mostrar último commit
        exit_code, output, error = exec_cmd(client, f"cd {APP_PATH} && git log -1 --oneline")
        log(f"📝 Commit: {output.strip()}", Colors.CYAN)
        
        # 3. Instalar dependencias
        log("📦 Verificando dependencias...", Colors.YELLOW)
        exit_code, output, error = exec_cmd(
            client, 
            f"cd {APP_PATH} && npm install --legacy-peer-deps 2>&1 | tail -5",
            timeout=600
        )
        log("✅ Dependencias verificadas", Colors.GREEN)
        
        # 4. Rebuild
        log("🏗️ Rebuilding aplicación...", Colors.YELLOW)
        log("⏳ Esto puede tomar unos minutos...", Colors.YELLOW)
        
        exit_code, output, error = exec_cmd(
            client, 
            f"cd {APP_PATH} && npm run build 2>&1 | tail -30",
            timeout=900
        )
        
        if exit_code != 0:
            log(f"❌ Error en build:\n{output}\n{error}", Colors.RED)
            exit_code2, output2, error2 = exec_cmd(
                client,
                f"cd {APP_PATH} && npm run build 2>&1 | grep -i 'error\\|failed' | head -20"
            )
            if output2:
                log(f"Errores detectados:\n{output2}", Colors.RED)
            return 1
        
        log("✅ Build completado", Colors.GREEN)
        
        # 5. Restart PM2
        log("♻️ Reiniciando aplicación...", Colors.YELLOW)
        
        exit_code, output, error = exec_cmd(client, "pm2 reload inmova-app")
        if exit_code != 0:
            exit_code, output, error = exec_cmd(client, "pm2 restart inmova-app")
        log("✅ PM2 reiniciado", Colors.GREEN)
        
        # 6. Esperar warm-up
        log("⏳ Esperando warm-up (20s)...", Colors.YELLOW)
        time.sleep(20)
        
        # 7. Health checks
        log("🏥 Verificando health...", Colors.YELLOW)
        
        exit_code, output, error = exec_cmd(
            client, 
            "curl -s -o /dev/null -w '%{http_code}' http://localhost:3000/propiedades --max-time 10"
        )
        status_propiedades = output.strip()
        
        exit_code, output, error = exec_cmd(
            client, 
            "curl -s -o /dev/null -w '%{http_code}' http://localhost:3000/api/units --max-time 10"
        )
        status_api = output.strip()
        
        print("\n" + "=" * 70)
        log("📊 RESULTADOS", Colors.CYAN)
        print("=" * 70)
        
        if status_propiedades in ['200', '302', '307']:
            log(f"✅ /propiedades: {status_propiedades}", Colors.GREEN)
        else:
            log(f"❌ /propiedades: {status_propiedades}", Colors.RED)
        
        if status_api in ['200', '401']:
            log(f"✅ /api/units: {status_api}", Colors.GREEN)
        else:
            log(f"❌ /api/units: {status_api}", Colors.RED)
        
        # Verificar PM2 status
        exit_code, output, error = exec_cmd(client, "pm2 status inmova-app | grep -E 'online|errored'")
        if 'online' in output.lower():
            log("✅ PM2: online", Colors.GREEN)
        else:
            log(f"⚠️ PM2: {output.strip()}", Colors.YELLOW)
        
        print("\n" + "=" * 70)
        log("🎉 DEPLOYMENT COMPLETADO", Colors.GREEN)
        print("=" * 70)
        
        print(f"""
Cambios desplegados:
  ✅ Funcionalidad de eliminar propiedades implementada
  ✅ Diálogo de confirmación añadido
  ✅ Botón eliminar en vista grid y lista

URLs:
  - Propiedades: http://{SERVER_IP}:3000/propiedades
  - Propiedades (dominio): https://inmovaapp.com/propiedades

Para verificar:
  1. Ir a https://inmovaapp.com/propiedades
  2. Hacer clic en el menú (⋮) de cualquier propiedad
  3. Seleccionar "Eliminar"
  4. Debería aparecer un diálogo de confirmación
""")
        
        return 0
        
    except Exception as e:
        log(f"❌ Error: {e}", Colors.RED)
        import traceback
        traceback.print_exc()
        return 1
    finally:
        client.close()
        log("🔒 Conexión SSH cerrada", Colors.CYAN)

if __name__ == "__main__":
    sys.exit(main())
