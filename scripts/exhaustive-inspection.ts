...
interface PageError {
  type: 'console' | 'javascript' | 'network' | 'hydration' | 'navigation';
  message: string;
  url?: string;
  statusCode?: number;
  timestamp: number;
...
interface InspectionResults {
  ...
  criticalIssues: Array<{ page: string; issue: string }>;
}
...
class ExhaustiveInspector {
  ...
  private baseURL = 'https://inmovaapp.com';
  ...
  async inspectPage(pageData: typeof PAGES_TO_INSPECT[0]): Promise<PageInspection> {
    ...
    try {
      ...
      inspection.httpStatus = response?.status() || null;
  ...
  async run() {
    ...
    const byCategory: Record<string, typeof PAGES_TO_INSPECT> = {};
    ...
    for (const [category, pages] of Object.entries(byCategory)) {
      ...
      for (const pageData of pages) {
        process.stdout.write(`  ${pageData.name.padEnd(35)}...`);
