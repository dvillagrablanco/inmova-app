...
async function main() {
  ...
  if (planProfesional) {
    planProfesional = await prisma.subscriptionPlan.update({
      ...
      data: {
        nombre: 'Profesional',
        descripcion:
          'Plan ideal para gestores y pequeñas empresas inmobiliarias. Incluye funcionalidades esenciales para una gestión profesional. Máx. 5 usuarios, 50 propiedades.',
        precioMensual: 149,
        maxUsuarios: 5,
        maxPropiedades: 50,
        modulosIncluidos: [
  ...
  console.log(
    '✓ Plan Profesional creado:',
    planProfesional.nombre,
    '-',
    planProfesional.precioMensual,
    '€/mes'
  );
  ...
  if (planEmpresa) {
    planEmpresa = await prisma.subscriptionPlan.update({
      ...
      data: {
        nombre: 'Empresa',
        descripcion:
          'Solución completa para empresas inmobiliarias en crecimiento. Incluye módulos avanzados y mayor capacidad. Máx. 15 usuarios, 200 propiedades.',
        precioMensual: 349,
        maxUsuarios: 15,
        maxPropiedades: 200,
        modulosIncluidos: [
  ...
  if (planEnterprise) {
    planEnterprise = await prisma.subscriptionPlan.update({
      ...
      data: {
        nombre: 'Enterprise',
        descripcion:
          'Solución enterprise para grandes corporaciones. Incluye todos los módulos, integraciones avanzadas y soporte dedicado. Usuarios y propiedades ilimitados.',
        precioMensual: 899,
        maxUsuarios: 999999,
  ...
  console.log(
    '✓ Plan Enterprise creado:',
    planEnterprise.nombre,
    '-',
    planEnterprise.precioMensual,
    '€/mes'
  );
  ...
  console.log('\nResumen:');
  console.log(
    `- ${planProfesional.nombre}: ${planProfesional.precioMensual}€/mes (${planProfesional.maxUsuarios} usuarios, ${planProfesional.maxPropiedades} propiedades)`
  );
  console.log(
    `- ${planEmpresa.nombre}: ${planEmpresa.precioMensual}€/mes (${planEmpresa.maxUsuarios} usuarios, ${planEmpresa.maxPropiedades} propiedades)`
  );
  console.log(
    `- ${planEnterprise.nombre}: ${planEnterprise.precioMensual}€/mes (usuarios y propiedades ilimitados)`
  );
}
