#!/usr/bin/env python3
"""
Verificar tablas en la BD de producción
"""
import sys
...
def main():
    ...
    try:
        ...
        status, out, err = exec_cmd(client, 
            f'''cd {APP_PATH} && export $(grep -v '^#' .env | xargs) && psql "$DATABASE_URL" -c "SELECT tablename FROM pg_tables WHERE schemaname = 'public' ORDER BY tablename" 2>&1'''
        )
        log(f"\nTablas existentes:\n{out}", Colors.CYAN)
        ...
        for table in ['subscription_plans', 'promo_coupons', 'partners', 'ewoorker_plans']:
            if table.lower() in existing_tables:
                status, out, err = exec_cmd(client, 
                    f'''cd {APP_PATH} && export $(grep -v '^#' .env | xargs) && psql "$DATABASE_URL" -c "SELECT COUNT(*) FROM {table}" 2>&1'''
                )
                # Extraer el número
                try:
                    count = [line.strip() for line in out.split('\n') if line.strip().isdigit()][0]
                    log(f"  {table}: {count} registros", Colors.CYAN)
                except:
