#!/usr/bin/env python3
"""
Script para desplegar la integración del Centro Financiero - con fix de conflictos
"""
import sys
...
def main():
    print(f"""
{Colors.CYAN}{'='*70}
🚀 DEPLOYMENT - INTEGRACIÓN CENTRO FINANCIERO (CON FIX)
{'='*70}{Colors.RESET}
...
""")
    ...
    try:
        ...
        log("🔧 Eliminando archivos conflictivos...", Colors.CYAN)
        conflicts = [
            "pages/api/sales-team/representatives/[id].ts",
            "pages/api/sales-team/representatives/index.ts",
            "pages/api/sales-team/representatives",
            "pages/api/sales-team",
        ]
        for f in conflicts:
            exec_cmd(client, f"cd {APP_PATH} && rm -rf {f}", timeout=10)
        log("✅ Archivos conflictivos eliminados", Colors.GREEN)

        # 3. Limpiar cache
        log("🧹 Limpiando cache...", Colors.CYAN)
        exec_cmd(client, f"cd {APP_PATH} && rm -rf .next/cache", timeout=30)
