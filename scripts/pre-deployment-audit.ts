...
class DeploymentAuditor {
  ...
  private execCommand(command: string, silent = false): string {
    try {
   ...
   */
  private auditEnvironmentVariables() {
    ...
    for (const varName of requiredVars) {
      if (!envContent.includes(`${varName}=`)) {
        this.addIssue({
          severity: 'ERROR',
          category: 'Environment',
          message: `Variable de entorno ${varName} no encontrada`,
          recommendation: `Agregar ${varName} al archivo .env`
        });
      } else {
   ...
   */
  private auditLargeFiles() {
    this.log('\n🔍 [6/8] Validando tamaño de archivos...', 'info');
    
    const maxFileSize = 100 * 1024 * 1024; // 100MB
    const problematicPatterns = ['core', '*.log', 'node_modules'];
