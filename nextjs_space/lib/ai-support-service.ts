...
export interface SupportResponse {
  message: string;
  category: 'technical' | 'billing' | 'feature_help' | 'bug_report' | 'account' | 'general' | 'tutorial';
  confidence: number;
  actionsTaken?: Array<{
    action: string;
    result: string;
  ...
  commonProblems: [
    {
      ...
      solution: 'La página que buscas no existe o fue movida. Verifica la URL o regresa al dashboard principal.',
      autofix: false
    },
    {
    ...
    },
    {
      ...
      solution: 'Hubo un problema al cargar los datos. Intenta refrescar la página. Si persiste, verifica tu conexión.',
      autofix: true,
      autofixAction: 'refresh'
    },
    {
      problem: 'Formulario no se envía',
      solution: 'Verifica que todos los campos requeridos estén completos y en el formato correcto.',
      autofix: false
    }
  ],
...
export async function processAISupportRequest(
  ...
  try {
    ...
    return {
      message: 'Lo siento, he tenido un problema al procesar tu solicitud. He creado un ticket de soporte que será atendido lo antes posible.',
      category: 'technical',
      confidence: 0,
...
// BÚSQUEDA EN FAQ
// ============================================================================
function findFAQMatch(userMessage: string): (typeof KNOWLEDGE_BASE.faq[0] & { confidence: number }) | null {
...
- Dispositivo: ${request.metadata?.deviceType || 'desconocido'}`;
    ...
    logger.info('🧠 LLM response generated', { category: result.category, confidence: result.confidence });
    ...
    return {
      category: 'general',
      response: 'Lo siento, he tenido un problema al procesar tu consulta. ¿Podrías reformularla o proporcionar más detalles?',
      confidence: 0,
...
async function createSupportTicket(
  ...
  try {
    ...
    if (request.message.toLowerCase().includes('urgente') || 
        request.message.toLowerCase().includes('emergencia')) {
      priority = 'urgent';
...
async function executeAutomatedActions(
  ...
  try {
    ...
    if (llmResponse.category === 'bug_report' || 
        request.message.toLowerCase().includes('urgente')) {
      actions.push({
...
// ============================================================================
function getRelatedDocumentation(category: string): Array<{ title: string; url: string; snippet: string }> {
