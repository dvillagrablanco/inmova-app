  ...
  const crearEvento = async () => {
    try {
      ...
      const payload = {
        ...
        recordatorioMinutos: formData.recordatorioActivo ? parseInt(formData.recordatorioMinutos) : undefined
      };
                  ...
                  {eventoSeleccionado && (
                    <>
                      {TIPOS_EVENTO.find(t => t.value === eventoSeleccionado.tipo)?.icon && (
                        (() => {
                          const Icon = TIPOS_EVENTO.find(t => t.value === eventoSeleccionado.tipo)!.icon;
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge 
                      variant="outline"
                      style={{ 
                        backgroundColor: PRIORIDADES.find(p => p.value === eventoSeleccionado.prioridad)?.color,
                        color: 'white',
                        borderColor: PRIORIDADES.find(p => p.value === eventoSeleccionado.prioridad)?.color
                      }}
                    >
                      Prioridad: {PRIORIDADES.find(p => p.value === eventoSeleccionado.prioridad)?.label}
                    </Badge>
                  </div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <p className="mt-1 text-sm">
                        {formatDate(new Date(eventoSeleccionado.fechaInicio), 
                    ...
                    {eventoSeleccionado.fechaFin && (
                      <div>
                        ...
                        <p className="mt-1 text-sm">
                          {formatDate(new Date(eventoSeleccionado.fechaFin), 
                            eventoSeleccionado.todoElDia ? 'PPP' : 'PPP p', 
                            { locale: es }
                          )}
                        </p>
                  ...
                  {(eventoSeleccionado.building || eventoSeleccionado.unit || eventoSeleccionado.tenant) && (
                    <div className="rounded-lg border p-3">
                      ...
                      <div className="space-y-1 text-sm">
                        {eventoSeleccionado.building && (
                          <p><strong>Edificio:</strong> {eventoSeleccionado.building.nombre}</p>
                        )}
                        {eventoSeleccionado.unit && (
                          <p><strong>Unidad:</strong> {eventoSeleccionado.unit.numero}</p>
                        )}
                        {eventoSeleccionado.tenant && (
                          <p><strong>Inquilino:</strong> {eventoSeleccionado.tenant.nombreCompleto}</p>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  {formData.recordatorioActivo && (
                    <div>
                      ...
                      <Select 
                        ...
                        onValueChange={(value) => setFormData({ ...formData, recordatorioMinutos: value })}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
