...
export default function PortalPropietarioPage() {
  ...
  const fetchData = async (ownerData: Owner) => {
    try {
      ...
      const [buildingsRes, unitsRes, contractsRes, paymentsRes, expensesRes, notificationsRes] = await Promise.all([
        fetch('/api/buildings'),
        fetch('/api/units'),
        fetch('/api/contracts'),
        fetch('/api/payments'),
      ...
      if (contractsRes.ok) {
        ...
        const filteredContracts = allContracts.filter((c: any) => 
          c.unit && buildingIds.includes(c.unit.buildingId)
        );
      ...
      if (expensesRes.ok && canViewGastos) {
        ...
        const filteredExpenses = allExpenses.filter((e: any) => 
          buildingIds.includes(e.buildingId)
        );
  ...
  const variacionGastos = gastosMesAnterior > 0 
    ? ((gastosMensual - gastosMesAnterior) / gastosMesAnterior) * 100 
    : 0;
      ...
      <main className="max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-8">
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Análisis detallado de {buildings.length} {buildings.length === 1 ? 'propiedad' : 'propiedades'}
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {(owner?.ownerBuildings.some(ob => ob.verIngresos) && owner?.ownerBuildings.some(ob => ob.verGastos)) && (
