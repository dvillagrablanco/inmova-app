            ...
            <div key={i} className="border rounded-lg p-5">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    ...
                    <p className={`text-2xl font-bold ${
                      contract.probability >= 70 ? 'text-green-600' :
                      contract.probability >= 50 ? 'text-yellow-600' :
                      'text-red-600'
                    }`}>
                  ...
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center ${
                    contract.recommendation === 'renovar' ? 'bg-green-100' :
                    contract.recommendation === 'negociar' ? 'bg-yellow-100' :
                    'bg-red-100'
                  }`}>
                    {contract.recommendation === 'renovar' ? <CheckCircle className="h-8 w-8 text-green-600" /> :
                     contract.recommendation === 'negociar' ? <Clock className="h-8 w-8 text-yellow-600" /> :
                ...
                <div className="flex flex-wrap gap-2">
                  {contract.factors.map((factor, j) => (
                    <span key={j} className={`px-2 py-1 rounded text-xs ${
                      factor.impact.startsWith('+') ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold text-green-600">€{contract.newRent}/mes</p>
                </div>
                <div className="text-right">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant={contract.recommendation === 'renovar' ? 'default' : 'outline'}>
                  Generar Propuesta
                </Button>
                <Button size="sm" variant="outline">Ver Historial</Button>
                <Button size="sm" variant="outline">Análisis Completo</Button>
              </div>
            </div>
          ))}
        </div>
            ...
            <div key={i} className="flex justify-between items-center border-b pb-3">
              ...
              <span className={`px-3 py-1 rounded-full text-xs ${
                ...
                {proposal.status === 'aceptada' ? 'Aceptada' :
                 proposal.status === 'pendiente' ? 'Pendiente respuesta' :
                 'Rechazada'}
              </span>
            </div>
          ))}
