  ...
  const handleCreate = async () => {
    try {
      const response = await fetch('/api/certificaciones', {
        ...
        body: JSON.stringify({
          ...newCert,
          consumoEnergetico: newCert.consumoEnergetico ? parseFloat(newCert.consumoEnergetico) : null,
            ...
            <div className="flex items-center justify-between">
              <Breadcrumb>
                <BreadcrumbList>
                  ...
                  <BreadcrumbSeparator />
                  <BreadcrumbItem><BreadcrumbPage>Certificaciones</BreadcrumbPage></BreadcrumbItem>
                </BreadcrumbList>
              </Breadcrumb>
                  ...
                  <div className="flex gap-1">
                    {['A', 'B', 'C', 'D', 'E', 'F', 'G'].map((cal) => (
                      <div key={cal} className="text-center">
                        <div className={`text-xs font-bold px-1 rounded ${getCalificacionColor(cal)}`}>{cal}</div>
                        <div className="text-xs text-muted-foreground">{calificacionesCount[cal] || 0}</div>
                      </div>
                    ))}
                  </div>
                </CardContent>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium">{format(new Date(cert.fechaEmision), 'dd/MM/yyyy', { locale: es })}</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">Vencimiento</p>
                        <p className="text-sm font-medium">{format(new Date(cert.fechaVencimiento), 'dd/MM/yyyy', { locale: es })}</p>
                      </div>
                    </div>
                    {cert.ahorroEstimado && (
                      <div className="mt-3 p-2 bg-green-50 rounded-md text-sm flex items-center gap-2">
                        <Euro className="h-4 w-4 text-green-600" />
                        <span className="text-green-700">Ahorro estimado: €{cert.ahorroEstimado.toLocaleString()}/año</span>
                      </div>
                    )}
                  </CardContent>
                </Card>
      ...
      <Dialog open={openNew} onOpenChange={setOpenNew}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4">
            <div>
              <Label>Unidad *</Label>
              <Select value={newCert.unitId} onValueChange={(v) => setNewCert({ ...newCert, unitId: v })}>
                <SelectTrigger><SelectValue placeholder="Seleccionar" /></SelectTrigger>
                <SelectContent>
                  {units.map((u) => (
                    <SelectItem key={u.id} value={u.id}>Unidad {u.numero} - {u.building?.nombre}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Número Certificado</Label><Input value={newCert.numeroCertificado} onChange={(e) => setNewCert({ ...newCert, numeroCertificado: e.target.value })} /></div>
              <div>
                <Label>Calificación *</Label>
                <Select value={newCert.calificacion} onValueChange={(v) => setNewCert({ ...newCert, calificacion: v })}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Consumo (kWh/m²/año)</Label><Input type="number" step="0.01" value={newCert.consumoEnergetico} onChange={(e) => setNewCert({ ...newCert, consumoEnergetico: e.target.value })} /></div>
              <div><Label>Emisiones CO₂ (kg/m²/año)</Label><Input type="number" step="0.01" value={newCert.emisionesCO2} onChange={(e) => setNewCert({ ...newCert, emisionesCO2: e.target.value })} /></div>
            </div>
            <div><Label>Nombre Técnico *</Label><Input value={newCert.nombreTecnico} onChange={(e) => setNewCert({ ...newCert, nombreTecnico: e.target.value })} /></div>
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Fecha Emisión *</Label><Input type="date" value={newCert.fechaEmision} onChange={(e) => setNewCert({ ...newCert, fechaEmision: e.target.value })} /></div>
              <div><Label>Fecha Vencimiento *</Label><Input type="date" value={newCert.fechaVencimiento} onChange={(e) => setNewCert({ ...newCert, fechaVencimiento: e.target.value })} /></div>
            </div>
            <div><Label>Recomendaciones</Label><Textarea value={newCert.recomendaciones} onChange={(e) => setNewCert({ ...newCert, recomendaciones: e.target.value })} rows={3} /></div>
            <div><Label>Ahorro Estimado (€/año)</Label><Input type="number" step="0.01" value={newCert.ahorroEstimado} onChange={(e) => setNewCert({ ...newCert, ahorroEstimado: e.target.value })} /></div>
          </div>
          <DialogFooter>
