...
export async function GET(request: NextRequest) {
  try {
    ...
    const activeProjects = projects.filter((p: any) =>
      ['EN_PROGRESO', 'EN_RENOVACION'].includes(p.estado || '')
    ).length;
    ...
    const totalInvestment = projects.reduce(
      (sum: number, p: any) => sum + (p.precioCompra || 0) + (p.gastosRealesRenovacion || 0),
      0
    );
    ...
    const completedProjectsWithROI = projects.filter(
      (p: any) => p.estado === 'VENDIDO' && p.precioVentaReal && p.precioCompra
    );
    const avgROI =
      completedProjectsWithROI.length > 0
        ? Math.round(
