...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey || !apiSecret) {
      return NextResponse.json({ error: 'API Key y API Secret son requeridos' }, { status: 400 });
    }
    ...
    if (!isValid) {
      return NextResponse.json({ error: 'Credenciales de Pomelli inválidas' }, { status: 400 });
    }
