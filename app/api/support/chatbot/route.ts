...
function generateResponse(query: string, results: any[], sentiment: any): any {
  ...
  // Pricing
    ...
    response =
      'INMOVA tiene un plan único de €149/mes para empresas con TODO incluido: 88 módulos, usuarios ilimitados, propiedades ilimitadas y soporte 24/7. También ofrecemos €49/mes para propietarios individuales. ¡Prueba gratis 30 días!';
    confidence = 0.95;
    intent = 'pricing';
  ...
  // Crear/Añadir
    ...
    if (
      lowerQuery.includes('edificio') ||
      lowerQuery.includes('propiedad') ||
      lowerQuery.includes('inmueble')
    ) {
      response =
        'Para crear un edificio, ve a Edificios > Nuevo Edificio. Completa los datos básicos y ¡listo! Te recomiendo ver el video tutorial para una guía paso a paso.';
      confidence = 0.9;
      intent = 'how_to';
      ...
      suggestedActions.push(
        ...
        },
        {
          id: '2',
          label: 'Ver tutorial',
          action: 'play_video:https://www.youtube.com/embed/zm55Gdl5G1Q',
          icon: 'BookOpen',
        }
      );
    } else if (
      lowerQuery.includes('unidad') ||
      lowerQuery.includes('apartamento') ||
      lowerQuery.includes('piso')
    ) {
      response =
        'Para añadir unidades, primero debes tener un edificio creado. Luego ve a Unidades > Nueva Unidad, selecciona el edificio y completa los datos.';
      confidence = 0.9;
      intent = 'how_to';
      ...
      response =
        'Para registrar un inquilino, ve a Inquilinos > Nuevo Inquilino. Completa sus datos personales y podrás asignarlo a una unidad disponible.';
      confidence = 0.9;
      intent = 'how_to';
      ...
      response =
        'Para crear un contrato, ve a Contratos > Nuevo Contrato. Selecciona el inquilino y la unidad, define las condiciones y genera el PDF automáticamente.';
      confidence = 0.9;
      intent = 'how_to';
      ...
      response =
        'Puedo ayudarte a crear edificios, unidades, inquilinos, contratos y más. ¿Qué te gustaría crear específicamente?';
      confidence = 0.6;
    }
  }
  // Problemas/Errores
    ...
    response =
      'Lamento que estés teniendo problemas. Para ayudarte mejor, ¿podrías especificar qué funcionalidad no está funcionando? Mientras tanto, revisa si tu sesión está activa y los permisos de tu usuario.';
    confidence = 0.7;
    intent = 'support';
  ...
  // Respuesta basada en búsqueda
    ...
    if (topResult.videoUrl) {
      suggestedActions.push({
        ...
        action: `play_video:${topResult.videoUrl}`,
        icon: 'BookOpen',
      });
    }
  }
  ...
  if (sentiment.urgency === 'high' || sentiment.urgency === 'critical') {
    suggestedActions.unshift({
      id: '0',
      label: 'Hablar con un humano AHORA',
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      message:
        'Lo siento, he tenido un problema técnico. Por favor, intenta de nuevo o contacta con el soporte.',
      confidence: 0,
      suggestedActions: [
        { id: '1', label: 'Crear ticket de soporte', action: 'create_ticket', icon: 'Ticket' },
