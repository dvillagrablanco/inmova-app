  ...
  try {
    ...
    const renewals = await prisma.contract.findMany({
      where: {
        ...
        fechaFin: { lte: new Date(Date.now() + 90 * 86400000) },
      },
      select: {
        id: true,
        rentaMensual: true,
    ...
    return NextResponse.json({
      success: true,
      approvals: {
        renovaciones: renewals.map((r) => ({
          id: r.id,
          tipo: 'renovacion',
          edificio: r.unit?.building?.nombre,
          unidad: r.unit?.numero,
          inquilino: r.tenant?.nombreCompleto,
          renta: r.rentaMensual,
          vencimiento: r.fechaFin,
        })),
        mantenimiento: maintenance.map((m) => ({
          id: m.id,
          tipo: 'mantenimiento',
          edificio: m.unit?.building?.nombre,
          unidad: m.unit?.numero,
          titulo: m.titulo,
          coste: m.costoEstimado,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!itemId || !tipo || !decision)
      return NextResponse.json({ error: 'itemId, tipo, decision required' }, { status: 400 });

    if (tipo === 'mantenimiento') {
      await prisma.maintenanceRequest.update({
        where: { id: itemId },
        data: { estadoAprobacion: decision === 'aprobar' ? 'aprobado' : 'rechazado' },
      });
    }

    return NextResponse.json({
      success: true,
      message: `${tipo} ${decision === 'aprobar' ? 'aprobado' : 'rechazado'}`,
    });
  } catch (error: any) {
