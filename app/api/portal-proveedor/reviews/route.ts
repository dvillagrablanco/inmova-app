...
export async function GET(request: NextRequest) {
  try {
    ...
    const avgCalidad =
      totalReviews > 0 ? reviews.reduce((sum, r) => sum + (r.calidad || 0), 0) / totalReviews : 0;
    const avgPuntualidad =
      totalReviews > 0
        ? reviews.reduce((sum, r) => sum + (r.puntualidad || 0), 0) / totalReviews
        : 0;
    const avgProfesionalismo =
      totalReviews > 0
        ? reviews.reduce((sum, r) => sum + (r.profesionalismo || 0), 0) / totalReviews
        : 0;
    const avgPrecio =
      totalReviews > 0 ? reviews.reduce((sum, r) => sum + (r.precio || 0), 0) / totalReviews : 0;
    const avgComunicacion =
      totalReviews > 0
