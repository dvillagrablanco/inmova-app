...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const buildings = await prisma.building.count({
      where: { companyId: { in: groupIds }, isDemo: false },
    });
    ...
    const rentaTotal = units
      .filter((u) => u.estado === 'ocupada')
      .reduce((s, u) => s + u.rentaMensual, 0);
    const valorInmobiliario = units.reduce(
      (s, u) => s + (u.valorMercado || u.precioCompra || 0),
      0
    );
    ...
    const valorFinanciero = accounts.reduce(
      (s, a) => s + a.positions.reduce((ps, p) => ps + p.valorActual, 0),
      0
    );
    const pnlFinanciero = accounts.reduce(
    ...
    const allocation = {
      inmobiliario:
        patrimonioTotal > 0 ? Math.round((valorInmobiliario / patrimonioTotal) * 1000) / 10 : 0,
      financiero:
        patrimonioTotal > 0 ? Math.round((valorFinanciero / patrimonioTotal) * 1000) / 10 : 0,
      privateEquity: patrimonioTotal > 0 ? Math.round((valorPE / patrimonioTotal) * 1000) / 10 : 0,
      liquidez:
        patrimonioTotal > 0 ? Math.round((saldoTesoreria / patrimonioTotal) * 1000) / 10 : 0,
    };

    // ── ALERTAS ──
    const alertas: string[] = [];
    if (allocation.inmobiliario > 60)
      alertas.push(
        `⚠️ Concentración inmobiliaria alta (${allocation.inmobiliario}%). Diversificar.`
      );
    if (allocation.liquidez < 5)
      alertas.push(`⚠️ Liquidez baja (${allocation.liquidez}%). Mantener mínimo 5% en efectivo.`);
    if (allocation.financiero < 15)
      alertas.push(
        `💡 Cartera financiera infrautilizada (${allocation.financiero}%). Considerar más inversión en fondos.`
      );
...
Sé directo, ejecutivo, con 1-2 recomendaciones.`,
    ...
    const report = {
      ...
      resumenEjecutivo:
        resumenIA || `Patrimonio total: ${Math.round(patrimonioTotal).toLocaleString('es-ES')}€`,
      patrimonio: {
      ...
      financiero: {
        ...
        posiciones: accounts.reduce((s, a) => s + a.positions.length, 0),
        pnl: Math.round(pnlFinanciero),
      },
