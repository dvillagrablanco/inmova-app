...
import { CustomizableDashboard, DashboardWidget } from '@/components/CustomizableDashboard';
...
function DashboardPageContent() {
  ...
  const [customizableMode, setCustomizableMode] = useState(false);
  ...
  // Generate widgets for customizable dashboard
  const dashboardWidgets: DashboardWidget[] = data ? [
    {
      id: 'kpis',
      title: 'Indicadores Clave',
      defaultLayout: { x: 0, y: 0, w: 12, h: 3, minH: 2 },
      component: (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <p className="text-sm text-muted-foreground">Ingresos</p>
            <p className="text-2xl font-bold">€{data.kpis.ingresosTotalesMensuales.toLocaleString('es-ES')}</p>
          </div>
          <div className="text-center">
            <p className="text-sm text-muted-foreground">Propiedades</p>
            <p className="text-2xl font-bold">{data.kpis.numeroPropiedades}</p>
          </div>
          <div className="text-center">
            <p className="text-sm text-muted-foreground">Ocupación</p>
            <p className="text-2xl font-bold">{data.kpis.tasaOcupacion}%</p>
          </div>
          <div className="text-center">
            <p className="text-sm text-muted-foreground">Morosidad</p>
            <p className="text-2xl font-bold">{data.kpis.tasaMorosidad}%</p>
          </div>
        </div>
      ),
    },
    {
      ...
      defaultLayout: { x: 0, y: 3, w: 6, h: 5, minH: 4 },
      component: (
    ...
    },
    {
      id: 'occupancy-chart',
      title: 'Ocupación',
      defaultLayout: { x: 6, y: 3, w: 6, h: 5, minH: 4 },
      component: (
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data.occupancyChartData}>
            ...
            <Bar dataKey="disponibles" stackId="a" fill="#7C3AED" />
          </BarChart>
        </ResponsiveContainer>
      ),
    },
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-8 flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold text-gray-900" data-tour="dashboard-kpis">Dashboard</h1>
                <p className="text-gray-600 mt-1">Bienvenido, {session?.user?.name || 'Usuario'}</p>
              </div>
              <Button
                variant={customizableMode ? "default" : "outline"}
                onClick={() => setCustomizableMode(!customizableMode)}
                className="gap-2"
              >
                <LayoutGrid className="h-4 w-4" />
                {customizableMode ? 'Modo Normal' : 'Personalizar Dashboard'}
              </Button>
            </div>
            ...
            {/* Conditional: Customizable Dashboard or Normal Dashboard */}
            {customizableMode ? (
              <CustomizableDashboard 
                widgets={dashboardWidgets} 
                storageKey={session?.user?.email || 'default'}
              />
            ) : (
              <>
                {/* KPIs Grid */}
