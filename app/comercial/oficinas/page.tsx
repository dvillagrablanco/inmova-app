...
interface Oficina {
  id: string;
  /** Present when list falls back to residential Unit rows */
  _source?: string;
  nombre: string;
  direccion: string;
  superficie: number;
...
export default function OficinasPage() {
  ...
  const detailHref = (o: Oficina) =>
    o._source === 'unit' ? `/propiedades/${o.id}` : `/comercial/espacios/${o.id}`;
  const editHref = (o: Oficina) =>
    o._source === 'unit' ? `/propiedades/${o.id}/editar` : `/comercial/espacios/${o.id}/editar`;

  const filteredOficinas = oficinas.filter((oficina) => {
    const matchesSearch =
      oficina.nombre.toLowerCase().includes(searchQuery.toLowerCase()) ||
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <div className="text-2xl font-bold">
                {stats.rentaMensualTotal.toLocaleString('es-ES')}€
              </div>
            </CardContent>
          </Card>
        </div>
                ...
                <div className="flex items-start justify-between">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => router.push(detailHref(oficina))}>
                        <Eye className="h-4 w-4 mr-2" />
                        Ver detalles
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => router.push(editHref(oficina))}>
        ...
        {filteredOficinas.length === 0 && (
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <p className="text-gray-600 mt-1">
                Prueba con otros filtros o añade una nueva oficina
              </p>
