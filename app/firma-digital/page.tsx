        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Gestiona documentos para firma electrónica (DocuSign / Signaturit)
            </p>
          </div>
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                      <div className="flex-1">
                        ...
                        <div className="ml-12 mt-2 space-y-1 text-sm text-muted-foreground">
                          ...
                          <p>📅 Creado: {format(new Date(doc.createdAt), 'PPP', { locale: es })}</p>
                          {doc.fechaExpiracion && (
                            <p
                              className={
                        ...
                        <TableRow key={doc.id}>
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2">
                              <Button variant="ghost" size="sm" onClick={() => verDetalle(doc.id)}>
                                <Eye className="h-4 w-4" />
                              </Button>
                              {doc.estado === 'pendiente' && (
                                <>
            ...
            {documentoSeleccionado && (
              <div className="space-y-6">
                ...
                <div className="rounded-lg bg-blue-50 p-4 dark:bg-blue-950/30">
                  <p className="text-sm">
                    <strong>DocuSign / Signaturit:</strong> Cada firmante recibe un enlace único por
                    email para firmar el documento de forma segura con validez legal.
                  </p>
                </div>
              </div>
            )}
        ...
        <Dialog open={mostrarDialogoNuevo} onOpenChange={setMostrarDialogoNuevo}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Crea un documento para firma electrónica</DialogDescription>
            </DialogHeader>

            <div className="space-y-4 py-2">
              <div>
                <Label htmlFor="titulo">Título del documento</Label>
                <Input id="titulo" placeholder="Ej: Contrato de arrendamiento Piso 3A" />
              </div>
              <div>
              ...
              <div>
                <Label htmlFor="firmante_nombre">Nombre del firmante</Label>
                <Input id="firmante_nombre" placeholder="Nombre completo" />
              </div>
              <div>
                <Label htmlFor="firmante_email">Email del firmante</Label>
                <Input id="firmante_email" type="email" placeholder="email@ejemplo.com" />
              </div>
            </div>

            <DialogFooter>
              <Button variant="outline" onClick={() => setMostrarDialogoNuevo(false)}>
                Cancelar
              </Button>
              <Button
                onClick={async () => {
                  const titulo = (document.getElementById('titulo') as HTMLInputElement)?.value;
                  const nombre = (document.getElementById('firmante_nombre') as HTMLInputElement)
                    ?.value;
                  const email = (document.getElementById('firmante_email') as HTMLInputElement)
                    ?.value;
                  if (!titulo || !nombre || !email) {
                    toast.error('Completa todos los campos');
                    return;
                  }
                  try {
                    const res = await fetch('/api/digital-signature', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({
                        titulo,
                        tipoDocumento: 'contrato',
                        firmantes: [{ nombre, email, rol: 'firmante' }],
                      }),
                    });
                    if (res.ok) {
