...
const MODULE_INFO: Record<
  string,
  { icon: any; title: string; description: string; gradient: string; route: string }
> = {
  dashboard: {
    icon: Home,
    title: 'Dashboard',
  ...
  unidades: {
    icon: Home,
    title: 'Unidades',
    description: 'Apartamentos y locales',
  ...
  mantenimiento: {
    icon: Hammer,
    title: 'Mantenimiento',
    description: 'Solicitudes y reparaciones',
  ...
  asistente_ia: {
    icon: Bot,
    title: 'Asistente IA',
    description: 'GPT-4 Conversacional',
    gradient: 'from-violet-500 to-purple-500',
    route: '/asistente-ia',
  },
  blockchain: {
  ...
  str_listings: {
    icon: Hotel,
    title: 'Anuncios STR',
    description: 'Propiedades turísticas',
  ...
  str_channels: {
    ...
    description: 'Sincronización canales',
  ...
  construction_projects: {
    icon: Building2,
    title: 'Construcción',
    description: 'Obras y desarrollo',
    gradient: 'from-orange-500 to-red-500',
  ...
  professional_projects: {
    ...
    description: 'Arquitectura y asesoría',
...
export default function HomePage() {
  ...
  // Check if user has seen onboarding (skip for super_admin)
  useEffect(() => {
    const hasSeenOnboarding = localStorage.getItem('hasSeenOnboarding');
    const isSuperAdmin = session?.user?.role === 'super_admin';
    if (!hasSeenOnboarding && session && !isSuperAdmin) {
  ...
  return (
    <>
      ...
      {!isSuperAdmin && <VerticalOnboardingTour />}
