...
import { Home, ArrowLeft, Image as ImageIcon, Video, Plus, Eye, Upload } from 'lucide-react';
...
function GaleriasPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [galeriasData, unitsData] = await Promise.all([galeriasRes.json(), unitsRes.json()]);
  ...
  const totalFotos = galerias.reduce(
    (sum, g) => sum + g.items.filter((i) => i.tipo === 'foto').length,
    0
  );
  const totalVideos = galerias.reduce(
    (sum, g) => sum + g.items.filter((i) => i.tipo === 'video').length,
    0
  );
