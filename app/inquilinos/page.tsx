...
function InquilinosPageContent() {
  ...
  const morosoTenants = tenants.filter(
    (tenant) => (tenant.estado || '').toLowerCase() === 'moroso'
  ).length;
  ...
  return (
    <AuthenticatedLayout>
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Smart Breadcrumbs */}
        <SmartBreadcrumbs totalCount={tenants.length} showBackButton={true} />
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="flex items-start gap-3">
            <div>
              ...
              <p className="text-muted-foreground">Gestiona los inquilinos de tus propiedades</p>
            </div>
            <ContextualHelp
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {tenants.length > 0 ? Math.round((activeTenants / tenants.length) * 100) : 0}% del
                total
              </p>
            </CardContent>
          </Card>
