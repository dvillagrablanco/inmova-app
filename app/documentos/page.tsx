...
export default function DocumentosPage() {
  ...
  const normalizeDocumentType = (value: string) => {
    ...
    if (normalized.includes('dni') || normalized.includes('documento')) return 'dni';
    if (normalized.includes('nomina') || normalized.includes('nómina')) return 'nomina';
    if (normalized.includes('certificado')) return 'certificado_energetico';
    if (normalized.includes('ite')) return 'ite';
  ...
  const handleApplyDocumentData = (data: Record<string, any>) => {
    const updates: Partial<typeof uploadForm> = {};
    const nombre =
      data.nombre || data.nombreDocumento || data.documentName || data.title || data.titulo;
    if (nombre) {
      updates.nombre = String(nombre);
    }

    const tipoRaw = data.tipo || data.tipoDocumento || data.documentType || data.category;
    if (tipoRaw) {
      const normalizedType = normalizeDocumentType(String(tipoRaw));
      if (normalizedType) {
        updates.tipo = normalizedType;
      }
    }

    const fecha =
      data.fechaVencimiento || data.fechaCaducidad || data.expirationDate || data.expiryDate;
    const parsedDate = toInputDate(fecha);
    if (parsedDate) {
      updates.fechaVencimiento = parsedDate;
    }
              ...
              <DialogContent>
                ...
                <form onSubmit={handleUpload} className="space-y-4">
                  <AIDocumentAssistant
                    ...
                    onApplyData={handleApplyDocumentData}
                  />
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      <div className="flex-1 space-y-3">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                          <h3 className="text-lg font-semibold break-words flex-1">{doc.nombre}</h3>
