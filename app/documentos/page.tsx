...
export default function DocumentosPage() {
  ...
  // Handler para OCR - Esta función se ejecuta después del OCR
  const handleOCRComplete = (result: any) => {
    try {
      // El OCR ya subió el archivo y extrajo los datos
      // Simplemente cerramos el dialog y recargamos la lista
      toast.success('Documento procesado correctamente con OCR');
      setOpenUploadDialog(false);
      setUploadForm({ file: null, nombre: '', tipo: 'otro', fechaVencimiento: '' });
      fetchDocuments();
    } catch (error) {
      logError(new Error('Error al procesar documento con OCR'), { originalError: error });
              ...
              {canCreate && (
                <Dialog open={openUploadDialog} onOpenChange={setOpenUploadDialog}>
                  ...
                  <DialogContent>
                    ...
                    {/* OCR Scanner Section */}
                    <div className="mb-6 p-4 bg-gradient-to-r from-orange-50 to-amber-50 dark:from-orange-950 dark:to-amber-950 rounded-lg border-2 border-orange-200 dark:border-orange-800">
                      ...
                      <p className="text-sm text-orange-700 dark:text-orange-300 mb-3">
                        Usa esta opción para extraer automáticamente información del documento
                      </p>
                      <DocumentUploaderOCR
                        documentType="otro"
                        onUploadComplete={handleOCRComplete}
                        title="Subir con OCR"
                        description="El documento se subirá y procesará automáticamente"
