  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const msg =
          errorData?.detail || errorData?.error || `Error al cargar documentos (${res.status})`;
        logger.error('Error fetching documents:', { status: res.status, error: errorData });
        setLoadError(msg);
        toast.error(msg);
      }
    } catch (error: any) {
      const msg = error?.message || 'Error de conexión al cargar documentos';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
          <Card className="border-red-200 bg-red-50/50">
            <CardContent className="p-8 text-center space-y-4">
              ...
              <h2 className="text-lg font-semibold text-red-800">Error al cargar documentos</h2>
                    ...
                    <div className="space-y-2">
                      ...
                      {selectedDocument.unit && (
                        <div className="flex items-center gap-2">
                          <Home className="h-4 w-4 text-primary" />
                          <span className="font-medium">
                            Unidad: {selectedDocument.unit.numero}
                          </span>
                        </div>
                      )}
                    </div>
