      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Card>
          ...
          <CardContent>
            ...
            {(() => {
              const v = data?.rentabilidad?.variacionIngresos || 0;
              const positive = v >= 0;
              return (
                <div className="flex items-center text-xs text-muted-foreground mt-1">
                  ...
                  {positive ? '+' : ''}
                  {v.toFixed(1)}% vs período anterior
                </div>
              );
            })()}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {data?.ocupacion?.length || 0} edificios analizados
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {data?.rentabilidad?.margenNeto?.toFixed(1) || '0.0'}%
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              Gastos: €
              {data?.rentabilidad?.gastosTotales?.toLocaleString('es-ES', {
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {data?.rentabilidad?.tasaMorosidad === 0
                ? 'Sin pagos atrasados'
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={data?.ingresos || []}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="rentas" fill="#4F46E5" name="Rentas" />
                  <Bar dataKey="servicios" fill="#7C3AED" name="Servicios" />
                  <Bar dataKey="otros" fill="#A78BFA" name="Otros" />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={data?.ocupacion || []}>
                  ...
                  <YAxis domain={[0, 100]} />
                  <Tooltip />
                  <Bar dataKey="porcentaje" fill="#3B82F6" name="% Ocupación" />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
              ...
              <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 bg-muted rounded-lg">
                  ...
                  <div className="text-xl font-bold mt-1 flex items-center gap-2">
                    {(data?.rentabilidad?.variacionIngresos || 0) >= 0 ? (
                      <>
                        <TrendingUp className="h-5 w-5 text-green-600" /> Positiva
                      </>
                    ) : (
                      <>
                ...
                <div className="p-4 bg-muted rounded-lg">
                  <div className="text-sm text-muted-foreground">Variación del Período</div>
                  <div className="text-xl font-bold mt-1">
                    {(data?.rentabilidad?.variacionIngresos || 0) >= 0 ? '+' : ''}
                    {(data?.rentabilidad?.variacionIngresos || 0).toFixed(1)}%
                  </div>
                </div>
                <div className="p-4 bg-muted rounded-lg">
                  ...
                  <div className="text-xl font-bold mt-1">
                    €
                    {data?.rentabilidad?.ingresosNetos?.toLocaleString('es-ES', {
                      minimumFractionDigits: 0,
                    }) || '0'}
                  </div>
                </div>
