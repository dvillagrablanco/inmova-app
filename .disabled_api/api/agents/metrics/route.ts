...
export async function GET(request: NextRequest) {
  try {
    ...
    // Obtener métricas
    const metrics = await getAgentsMetrics(
      agentType || undefined,
      periodDays
    );
    ...
    const globalSummary = {
      ...
      avgConfidence: metrics.length > 0
        ...
        : 0,
      avgEscalationRate: metrics.length > 0
        ? metrics.reduce((sum, m) => sum + m.escalationRate, 0) / metrics.length
        : 0,
      totalSuccessRate: metrics.reduce((sum, m) => sum + m.successfulInteractions, 0) /
                        Math.max(metrics.reduce((sum, m) => sum + m.totalInteractions, 0), 1) * 100
    };

    // Agente más usado
    const mostUsedAgent = metrics.reduce((max, m) =>
      m.totalInteractions > max.totalInteractions ? m : max,
      metrics[0]
    );

    // Agente con mejor rendimiento
    const bestPerformingAgent = metrics.reduce((best, m) =>
      m.averageConfidence > best.averageConfidence ? m : best,
      metrics[0]
    );
