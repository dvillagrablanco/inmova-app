...
export async function predictEquipmentFailures(companyId: string) {
  ...
  for (const [equipo, stats] of equipmentStats) {
    ...
    predictions.push({
      ...predictionRecord,
      probabilidad: predictionRecord.probabilidadFalla,
      intervaloPromedio: Math.round(intervaloPromedio),
      factoresRiesgo,
      recomendaciones,
      nivelConfianza: predictionRecord.confianza,
      fechaEstimada: fechaObjetivo,
    });
