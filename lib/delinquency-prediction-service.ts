 ...
 */
export async function predictDelinquencyRisk(
  tenantId: string
...
async function extractFeatures(tenant: any, contract: any): Promise<TenantFeatures> {
  ...
  const missedPayments = payments.filter((p: any) => p.estado === 'pendiente' || p.estado === 'vencido').length;
  ...
  const avgDaysLate = latePaymentDays.length > 0
 ...
 */
function calculateRiskScore(features: TenantFeatures): Omit<DelinquencyPrediction, 'tenantId' | 'tenantName'> {
  let riskScore = 0;
  const weights = {
    paymentHistory: 0.4,
    tenantProfile: 0.25,
  ...
  if (features.paymentHistory.totalPayments > 0) {
    ...
    const missedRate = features.paymentHistory.missedPayments / features.paymentHistory.totalPayments;
  ...
  const depositCoverage = features.currentContract.depositAmount / features.currentContract.monthlyRent;
  ...
  if (features.economic.rentToIncomeRatio) {
    if (features.economic.rentToIncomeRatio > 35) {
      economicScore += 40;
      recommendations.push(`Renta es ${features.economic.rentToIncomeRatio.toFixed(0)}% del ingreso (> 35%)`);
 ...
 */
export async function predictDelinquencyForAll(companyId: string): Promise<DelinquencyPrediction[]> {
  try {
