...
function getJwtSecret(): string {
  ...
  if (!secret) {
    throw new Error('JWT_SECRET environment variable is required. Provider portal authentication will not work without it.');
  }
  return secret;
}
...
export function verifyProviderToken(token: string): ProviderTokenPayload | null {
  try {
    const decoded = jwt.verify(token, getJwtSecret()) as ProviderTokenPayload;
    return decoded;
  } catch (error) {
    return null;
  }
