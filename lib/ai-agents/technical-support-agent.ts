...
const tools: AgentTool[] = [
  {
    ...
    description:
      'Crea una nueva solicitud de mantenimiento en el sistema. Requiere título, descripción y puede incluir prioridad, categoría y unidad.',
    inputSchema: {
      type: 'object',
      properties: {
        titulo: {
    ...
    handler: async (input, context) => {
      try {
        ...
        if (!unitId) {
          return {
            error:
              'No se pudo determinar la unidad. Por favor, especifica el número de unidad o edificio.',
          };
        }
  ...
  },
  {
    ...
    handler: async (input, context) => {
      const request = await prisma.maintenanceRequest.findFirst({
        where: {
          OR: [{ id: input.requestId }, { id: { startsWith: input.requestId.toLowerCase() } }],
          unit: {
            building: {
              companyId: context.companyId,
            },
  ...
  },
  {
    name: 'diagnose_issue',
    description:
      'Proporciona un diagnóstico preliminar y posibles soluciones para problemas comunes',
    inputSchema: {
      type: 'object',
      properties: {
        problema: {
    ...
    handler: async (input, context) => {
      ...
      return {
        ...
        recomendacion:
          'Basándome en la descripción, estas son las causas más comunes. Se recomienda crear una solicitud de mantenimiento para una evaluación profesional.',
  ...
  },
  {
    ...
    handler: async (input, context) => {
      ...
      if (!providerId) {
        return {
          error:
            'No se encontró un proveedor disponible para esta categoría. Por favor, asigna manualmente.',
        };
      }
    ...
    fontaneria: [
      {
        ...
        causas: ['Junta deteriorada', 'Tubería rota', 'Conexión floja'],
        solucion: 'Requiere inspección de plomero',
      },
      {
        problema: 'Baja presión',
        causas: ['Válvula parcialmente cerrada', 'Tubería obstruida', 'Fuga interna'],
        solucion: 'Revisar válvulas y tuberías',
      },
      {
        problema: 'Drenaje lento',
        causas: ['Obstrucción parcial', 'Acumulación de residuos'],
        solucion: 'Limpieza de drenaje',
      },
    ],
    electricidad: [
      {
        problema: 'Apagones frecuentes',
        causas: ['Sobrecarga del circuito', 'Breaker defectuoso'],
        solucion: 'Revisión eléctrica urgente',
      },
      {
        problema: 'Luz parpadeante',
        causas: ['Conexión floja', 'Lámpara defectuosa', 'Problema en el cableado'],
        solucion: 'Revisar conexiones y bombillas',
      },
      {
        problema: 'Toma sin corriente',
        causas: ['Breaker desconectado', 'Cable roto', 'Toma dañada'],
        solucion: 'Revisar panel eléctrico',
      },
    ],
    climatizacion: [
      {
        problema: 'AC no enfría',
        causas: ['Gas refrigerante bajo', 'Filtro sucio', 'Compresor dañado'],
        solucion: 'Mantenimiento de AC',
      },
      {
        problema: 'Ruido excesivo',
        causas: ['Rodamientos desgastados', 'Suciedad en ventilador'],
        solucion: 'Limpieza y lubricación',
      },
    ],
  };

  return (
    baseDiagnosticos[categoria || 'otro'] || [
      {
        problema: 'Diagnóstico general',
        causas: ['Requiere inspección'],
        solucion: 'Evaluación profesional necesaria',
      },
    ]
  );
}
