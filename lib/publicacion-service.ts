 ...
 */
export async function generarAnunciosMultiplataforma(unitId: string): Promise<AnuncioGenerado[]> {
 ...
 */
async function generarAnuncioParaPortal(unit: any, portal: string): Promise<AnuncioGenerado> {
...
export async function guardarPublicacion(
  ...
  return await prisma.publicacionPortal.create({
    data: {
      ...
      fianza: datosAdicionales?.precioAlquiler
        ? datosAdicionales.precioAlquiler * 2
        : unit.rentaMensual * 2,
