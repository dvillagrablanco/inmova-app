...
export function analyzeDeal(input: DealAnalysisInput): DealAnalysisResult {
  ...
  const closingCostsPurchase = input.purchase.closingCosts || purchasePrice * 0.10; // 10% por defecto
  const totalPurchaseCost = purchasePrice + closingCostsPurchase;
  
  // 2. COSTES DE RENOVACIÓN
  const renovationCosts = input.renovation;
  const totalRenovationCost = Object.values(renovationCosts).reduce((sum, cost) => sum + (cost || 0), 0);
  
  // 3. COSTES DE MANTENIMIENTO (HOLDING)
  const holdingCostsPerMonth = Object.values(input.holding).reduce((sum, cost) => sum + (cost || 0), 0);
  ...
  const totalSellingCost = agentCommission + closingCostsSale + staging + marketing + otherSellingCosts;
  
  // 5. INVERSIÓN TOTAL Y BENEFICIO
  const totalInvestment = totalPurchaseCost + totalRenovationCost + totalHoldingCost + totalSellingCost;
  ...
  // 8. REGLA DEL 70%
  const maxPurchasePrice70Rule = (input.afterRepairValue * 0.70) - totalRenovationCost;
...
function calculateMonthlyBreakdown(
  ...
  const monthlyLoanPayment = loanAmount > 0 && annualInterestRate > 0
...
function calculateSensitivity(input: DealAnalysisInput): SensitivityAnalysis {
  const scenarios = [
    ...
    { label: 'Escenario Conservador (-5% ARV, +15% Renovación)', arvChange: -5, renovationChange: 15 },
  ];
  ...
  const results = scenarios.map(scenario => {
    ...
    const totalRenovation = Object.values(input.renovation).reduce((sum, cost) => sum + (cost || 0), 0);
...
export function validateDeal(analysis: DealAnalysisResult): {
  ...
  if (!analysis.is70RuleCompliant) {
    ...
    recommendations.push(`Negociar precio de compra hasta máximo €${analysis.maxPurchasePrice70Rule.toLocaleString()}`);
  }
  
  // Validar margen de beneficio (20% recomendado)
  if (analysis.profitMargin < 20) {
    warnings.push(`Margen de beneficio bajo (${analysis.profitMargin}%). Se recomienda mínimo 20%.`);
  }
  ...
  if (analysis.totalProjectMonths > 12) {
    warnings.push(`Proyecto muy largo (${analysis.totalProjectMonths} meses). Aumentan riesgos y costes.`);
  ...
  if (analysis.roi >= 15 && analysis.is70RuleCompliant && analysis.profitMargin >= 20) {
    ...
    recommendations.push(`⚠️ Deal aceptable pero con riesgos. Considerar negociar mejores condiciones.`);
  }
...
${analysis.sensitivity?.scenarios.map(s => `
### ${s.label}
...
export function calculate70RuleMaxOffer(arv: number, renovationCosts: number): number {
  return roundToTwo((arv * 0.70) - renovationCosts);
}
