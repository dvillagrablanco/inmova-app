...
export async function syncAllICalFeeds(
  ...
  try {
    ...
    logger.info(`[CRON] Sincronizando ${channels.length} calendarios iCal...`);
      ...
      try {
        if (channel.url) {
          ...
          if (result.success) {
            ...
            logger.info(`  ✅ ${channel.canal} (${channel.listing.titulo}): ${result.eventsProcessed} eventos`);
          } else {
    ...
    logger.info(`[CRON] Sincronización completada en ${duration}ms`);
...
export async function syncAvailabilityToChannels(
  ...
  try {
    ...
    logger.info(`[CRON] Sincronizando disponibilidad de ${listings.length} listings...`);
    ...
    for (const listing of listings) {
      try {
        ...
        if (result.success) {
          ...
          logger.info(`  ✅ ${listing.titulo}: ${result.channelsSynced.join(', ')}`);
        } else {
    ...
    const duration = Date.now() - startTime;
    logger.info(`[CRON] Sincronización de disponibilidad completada en ${duration}ms`);
...
export async function autoCreateCleaningTasks(
  ...
  try {
    ...
    logger.info(`[CRON] Creando tareas de limpieza para ${bookings.length} checkouts...`);
    ...
    for (const booking of bookings) {
      try {
        ...
        itemsProcessed++;
        logger.info(`  ✅ Tarea creada para ${booking.listing.titulo} (checkout ${booking.checkOutDate.toLocaleDateString()})`);
      } catch (error) {
    ...
    const duration = Date.now() - startTime;
    logger.info(`[CRON] Creación de tareas completada en ${duration}ms`);
...
export async function sendAutomaticReviewRequests(
  ...
  try {
    ...
    logger.info(`[CRON] Enviando solicitudes de reseña a ${bookings.length} huéspedes...`);
    ...
    for (const booking of bookings) {
      try {
        ...
        if (result.sent) {
          itemsProcessed++;
          logger.info(`  ✅ Solicitud enviada a ${booking.guestEmail}`);
        } else {
    ...
    const duration = Date.now() - startTime;
    logger.info(`[CRON] Envío de solicitudes completado en ${duration}ms`);
...
export async function checkLegalCompliance(
  ...
  try {
    ...
    logger.info(`[CRON] Verificando cumplimiento legal de ${listings.length} listings...`);
    ...
    for (const listing of listings) {
      try {
        ...
        if (!validation.valid || validation.warnings.length > 0) {
          itemsProcessed++;
          logger.info(`  ⚠️  ${listing.titulo}:`);
          validation.warnings.forEach(w => logger.info(`      - ${w}`));
          
          // En producción, aquí se enviaría un email de alerta
          // await sendComplianceAlert(listing, validation);
        } else {
          logger.info(`  ✅ ${listing.titulo}: Cumplimiento OK`);
        }
      } catch (error) {
    ...
    const duration = Date.now() - startTime;
    logger.info(`[CRON] Verificación de cumplimiento completada en ${duration}ms`);
...
export async function executeCronJob(
  ...
  logger.info(`[CRON] Ejecutando trabajo: ${jobId}`);
