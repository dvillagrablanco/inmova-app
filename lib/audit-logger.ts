...
export type AuditAction =
  ...
  | 'PAYMENT_CREATED' | 'PAYMENT_MARKED_PAID' | 'PAYMENT_DELETED'
  ...
  | 'INSURANCE_CREATED' | 'INSURANCE_UPDATED'
  | 'DOCUMENT_UPLOADED' | 'DOCUMENT_DELETED' | 'DOCUMENT_DOWNLOADED'
  | 'EXPORT_DATA' | 'IMPORT_DATA'
  | 'ADMIN_ACTION' | 'PERMISSION_CHANGED' | 'PLAN_CHANGED'
  | 'API_KEY_CREATED' | 'API_KEY_REVOKED'
  | 'SETTINGS_CHANGED' | 'MFA_ENABLED' | 'MFA_DISABLED'
...
export function auditApiAction(
  ...
  return audit({
    ...
    userAgent: request?.headers ? new Headers(request.headers).get('user-agent') || undefined : undefined,
  });
}
