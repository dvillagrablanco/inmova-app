...
async function notifyCommissionsGenerated(
  ...
  try {
    ...
    for (const [salesRepId, comisionesList] of comercialesComisiones) {
      ...
      logger.info(`[AUTOMATION] Email enviado a ${salesRep.email}`);
    }
    
    logger.info('[AUTOMATION] Notificaciones de comisiones enviadas exitosamente');
  } catch (error) {
...
async function generateCommissionsReport(
  ...
  try {
    ...
    logger.info('[AUTOMATION] Reporte de comisiones enviado a administradores');
  } catch (error) {
...
export async function sendPartnerWelcomeEmail(salesRepId: string) {
  try {
    ...
    logger.info(`[AUTOMATION] Email de bienvenida enviado a ${salesRep.email}`);
  } catch (error) {
...
export async function sendMonthlyGoalsReminder() {
  try {
    logger.info('[AUTOMATION] Enviando recordatorios de objetivos mensuales...');
    ...
    logger.info('[AUTOMATION] Recordatorios de objetivos enviados');
  } catch (error) {
...
export async function updateAllSalesRepsMetrics() {
  try {
    logger.info('[AUTOMATION] Actualizando métricas de comerciales...');
    ...
    logger.info(`[AUTOMATION] Métricas actualizadas para ${salesReps.length} comerciales`);
  } catch (error) {
