...
// ============================================
// GENERADOR DE PDFs DE CONTRATOS
// ============================================

/**
 * Interfaz para datos de generación de contrato
 */
...
/**
 * Reemplaza las variables en una plantilla HTML
 * Ejemplo: {{INQUILINO_NOMBRE}} -> "Juan Pérez"
 */
function replaceTemplateVariables(html: string, variables: Record<string, any>): string {
  let result = html;
...
/**
 * Genera un PDF desde una plantilla HTML de contrato
 * 
 * @param templateData Datos de la plantilla y variables
...
export const generateContractPDF = async (
  ...
  try {
    ...
    const htmlContenido = replaceTemplateVariables(
      templateData.contenidoHTML,
    ...
    // Agregar header con logo si existe
    let yPos = margin;
    if (templateData.companyInfo?.logoUrl) {
      try {
        // Agregar logo (en producción, cargar la imagen real)
        doc.setFontSize(10);
        doc.text(templateData.companyInfo.nombre || 'INMOVA', margin, yPos);
        yPos += 15;
      } catch (error) {
        logger.warn('[PDF] No se pudo cargar el logo de la empresa');
      }
    }
    ...
    doc.setFontSize(18);
    doc.setFont('helvetica', 'bold');
    doc.text(templateData.titulo, pageWidth / 2, yPos, { align: 'center' });
    yPos += 15;
    ...
    // En producción, usar una librería como html2canvas o html-pdf
    const textContent = htmlContenido
      .replace(/<[^>]*>/g, '\n') // Remover tags HTML
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      if (templateData.companyInfo) {
        const footerText = [
          templateData.companyInfo.nombre,
          templateData.companyInfo.cif ? `CIF: ${templateData.companyInfo.cif}` : '',
          templateData.companyInfo.email || ''
        ].filter(Boolean).join(' | ');
    ...
    // Convertir a buffer
    const pdfBuffer = Buffer.from(doc.output('arraybuffer'));
    logger.info('[PDF] Contrato generado exitosamente', {
    ...
    logError(error, { message: '[PDF] Error al generar PDF de contrato' });
    throw new Error(`Error al generar PDF de contrato: ${error.message}`);
  }
};

/**
 * Genera el nombre de archivo para un contrato
 */
export const generateContractFilename = (
  ...
  const tenantClean = nombreTenant.replace(/\s+/g, '_').replace(/[^a-zA-Z0-9_]/g, '');
  const tipoClean = tipoContrato.replace(/\s+/g, '_');
  
  return `contrato_${tipoClean}_${tenantClean}_${fechaStr}.pdf`;
};
