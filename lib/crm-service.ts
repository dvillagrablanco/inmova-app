...
// ----------------------------------------------------------------------------
// Compatibilidad con APIs legacy (Lead model "lead" vs "cRMLead")
// ----------------------------------------------------------------------------
// Algunas rutas API usan nombres antiguos. Exponemos aliases para evitar fallos
// de build y mantener compatibilidad hacia atrás.

export function calculateLeadScoring(input: any): number {
  // Heurística simple basada en los campos que existen en las rutas legacy.
  // Si el input no encaja, devolvemos un score neutral.
  const presupuesto =
    typeof input?.presupuestoMensual === 'number'
      ? input.presupuestoMensual
      : input?.presupuestoMensual
        ? Number(input.presupuestoMensual)
        : 0;

  const urgencia = String(input?.urgencia || '').toLowerCase();
  const verticales = Array.isArray(input?.verticalesInteres) ? input.verticalesInteres : [];

  const factors: LeadScoringFactors = {
    hasCompanyWebsite: false,
    hasCompanyLinkedIn: false,
    hasIndustry: false,
    companySize: undefined,
    hasPhone: !!input?.telefono,
    hasJobTitle: !!input?.cargo,
    isDecisionMaker: false,
    emailsOpened: 0,
    emailsClicked: 0,
    callsMade: 0,
    meetingsHeld: 0,
    hasBudget: presupuesto > 0,
    hasAuthority: false,
    hasNeed: verticales.length > 0,
    hasTimeline: !!urgencia,
  };

  // Ajustes básicos por presupuesto/urgencia
  let score = calculateLeadScore(factors);
  if (presupuesto >= 500) score += 10;
  if (presupuesto >= 2000) score += 10;
  if (['alta', 'high', 'urgente'].includes(urgencia)) score += 10;
  if (verticales.length >= 2) score += 5;

  return Math.min(score, 100);
}

export function calculateProbabilidadCierre(input: any): number {
  const score = calculateLeadScoring(input);
  // Mapeo simple score->probabilidad (0..100)
  return Math.max(0, Math.min(100, Math.round(score)));
}

export function determinarTemperatura(scoreOrInput: any): 'frio' | 'templado' | 'caliente' {
  const score =
    typeof scoreOrInput === 'number' ? scoreOrInput : calculateLeadScoring(scoreOrInput);
