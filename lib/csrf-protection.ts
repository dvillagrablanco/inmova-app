...
// Secret para firmar tokens CSRF (OBLIGATORIO en .env - no usar fallback inseguro)
const CSRF_SECRET = (() => {
  const secret = process.env.CSRF_SECRET;
  if (!secret || secret.trim() === '') {
    throw new Error(
      'CSRF_SECRET environment variable is required. Add it to .env or .env.production'
    );
  }
  return secret;
})();
