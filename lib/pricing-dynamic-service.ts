...
function getUrgencyMultiplier(daysUntilBooking: number): number {
  ...
  if (daysUntilBooking < 1) return 0.8; // -20% (mismo día)
  if (daysUntilBooking < 3) return 0.9; // -10% (última hora)
 ...
 */
export async function applyDynamicPricing(unitId: string, date: Date): Promise<void> {
  try {
