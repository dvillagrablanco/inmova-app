...
export async function sendEmail(options: EmailOptions): Promise<boolean> {
  try {
    ...
    if (process.env.NODE_ENV === 'development' || !process.env.SMTP_USER) {
      logger.info('[EMAIL] (DEMO) Email que se enviaría a:', options.to);
      logger.info('[EMAIL] (DEMO) Asunto:', options.subject);
      return true;
    }
    ...
    logger.info('[EMAIL] Email enviado exitosamente a:', options.to);
    return true;
  } catch (error) {
