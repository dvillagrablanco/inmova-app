...
const BASE_URL_FOR_TESTS = BASE_URL || 'http://localhost:3000';
...
describeIf('🔥 Smoke Tests - Páginas Críticas', () => {
  describe('Páginas públicas cargan correctamente', () => {
    CRITICAL_PAGES.filter((p) => !p.requiresAuth).forEach((page) => {
      it(`${page.name} (${page.path}) debe cargar sin errores`, async () => {
        const response = await fetch(`${BASE_URL_FOR_TESTS}${page.path}`);
  ...
  describe('APIs críticas responden', () => {
    it('Health check responde OK', async () => {
      const response = await fetch(`${BASE_URL_FOR_TESTS}/api/health`);
    ...
    it('API de planes públicos responde', async () => {
      const response = await fetch(`${BASE_URL_FOR_TESTS}/api/public/subscription-plans`);
      expect(response.status).toBe(200);
    ...
    it('API de auth session responde', async () => {
      const response = await fetch(`${BASE_URL_FOR_TESTS}/api/auth/session`);
      expect(response.status).toBe(200);
    });
  });
});

describeIf('🔍 Verificación de Rutas Dinámicas', () => {
  it('Estructura de carpetas para rutas dinámicas existe', async () => {
    ...
    const response = await fetch(`${BASE_URL_FOR_TESTS}/admin/clientes/${testId}/editar`, {
      redirect: 'manual',
    });
