...
test.describe('Flujo Completo DNI → Formulario', () => {
  ...
  test('DNI-FLOW: Debe mostrar datos extraídos y permitir aplicarlos al formulario', async ({ page }) => {
    ...
    const acceptCookies = page.locator('button:has-text("Aceptar todas"), button:has-text("Accept")');
    ...
    // ========================================
    // PASO 3: Abrir el Asistente IA
    // ========================================
    console.log('📍 PASO 3: Abriendo asistente IA...');
    
    // Buscar el botón flotante de IA
    const aiButton = page.locator('button.rounded-full').filter({ 
    ...
    await page.screenshot({ path: 'test-results/flow-02-asistente-abierto.png', fullPage: true });
    console.log('✅ Asistente IA abierto');
    ...
    // Buscar el botón de seleccionar archivos
    const selectButton = page.locator('button:has-text("Seleccionar archivos"), span:has-text("Seleccionar archivos")').first();
    ...
    // Subir archivo usando file chooser
    ...
    // ========================================
    // PASO 5: Esperar respuesta de la API
    // ========================================
    console.log('📍 PASO 5: Esperando respuesta del API...');
    ...
    try {
      ...
      if (apiResponse.ok()) {
        ...
        console.log(`✅ Campos extraídos: ${extractedData.extractedFields?.length || 0}`);
        
        if (extractedData.extractedFields) {
          console.log('📋 Datos extraídos:');
          extractedData.extractedFields.forEach((field: any) => {
            console.log(`   - ${field.fieldName}: ${field.fieldValue} (${Math.round(field.confidence * 100)}%)`);
          });
        }
      }
    } catch (e: any) {
    ...
    const fileCard = page.locator('[class*="Card"]').filter({
    ...
    if (await fileCard.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await page.screenshot({ path: 'test-results/flow-05-archivo-seleccionado.png', fullPage: true });
    } else {
    ...
    if (await resultsPanel.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await page.screenshot({ path: 'test-results/flow-06-resultados-visibles.png', fullPage: true });
    } else {
    ...
    const reviewButton = page.locator('button:has-text("Revisar y aplicar"), button:has-text("Revisar datos"), button:has-text("Aplicar datos")').first();
    ...
    if (await reviewButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const reviewDialog = page.locator('[role="dialog"]').filter({
      ...
      const viewButton = page.locator('button:has-text("Ver datos"), button:has-text("Ver detalle")').first();
